...
describe('AsymmetricEncryptionHelper', () => {
    describe('#asymmetric en- and decryption', () => {
        it('should encrypt a data item using an asymmetric RSA key pair, and then decrypt it again, such that the decrypted data is the same as the input', (done) => {
            ...
            var encryptedAsym = AsymmetricEncryptionHelper_1.AsymmetricEncryptionHelper.encryptStringWithRsaPublicKey(data, x.publicKey);
            var decryptedAsym = AsymmetricEncryptionHelper_1.AsymmetricEncryptionHelper.decryptStringWithRsaPrivateKey(encryptedAsym, x.privateKey);
